...
class Dataset(object):
    ...
    def load_spec_list(self, fname):
        with open(self.cmd_args.data_root + '/CrCi/' + fname, 'r') as f:
    ...
    def setup(self, classname):
        ...
        if self.cmd_args.single_sample is None:

            with open(self.cmd_args.data_root + '/' + self.cmd_args.file_list, 'r') as f:
                for row in f:
                    self.file_names.append(row.strip())
                    self.load_spec_list(row.strip())
        else:
            self.file_names.append(self.cmd_args.single_sample)
            self.load_spec_list(self.cmd_args.single_sample)
