...
class CEHolder(object):
    ...
    # More importance on failed ces
    def weigh_failed_ce(self, expr_root, w=1):
        for ce in self.all_ces:
            if ce.check(expr_root) == "bad":
                # I believe here we are trying to generate more tests that are similar to the counterexample so we can weigh this area of examples heavier. Problem is, all neighbor tests
                # are included at the moment.

                #examples = self.sample.spectree.gen_neighbor_tests(w, ce.config)
                #print(examples)
                #assert False
                #self.additional_ces.extend(examples)

                print (f"Found an counterexample {ce.config}")
                #for ex in examples:
                #    print (f"Add an example {ex}")

    def warmup2(self):
        Ts,Fs = self.sample.spectree.all_tests
        #print(Ts, " are the trues", Fs, " are the falses")
        #assert False
        ...
        for x in ss:
            ...
            #print(ce.to_ce_str())
            # TODO: is ce.to_ce_str() poorly written?
    ...
    def eval_both(self, expr_root):
        #print("Expressoin root", expr_root)
        passed_ct = 0
        all_ct = 0
        # TODO: rewriting this
        for ce in self.all_ces:
            if ce.check(expr_root) == "good":
                passed_ct += 1
        return (passed_ct, len(self.all_ces))
        #print("length of cexpls", len(self.ce_per_key))
        #for key in self.ce_per_key:
        #    mem = self.ce_per_key[key]
        #    all_ct += mem.count
        #    print("Lengthof this mem", len(mem.ce_list))
