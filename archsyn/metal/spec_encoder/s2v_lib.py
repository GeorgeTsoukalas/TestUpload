...
class S2VGraph(object):
    def __init__(self, pg, node_type_dict):
        self.pg = pg
        print("Pg is " + str(pg))
        self.typed_edge_list = [None] * NUM_GRAMMAR_EDGE_TYPES
        for i in range(NUM_GRAMMAR_EDGE_TYPES):
            self.typed_edge_list[i] = []
        print("Typed_edge_list is " + str(self.typed_edge_list))
        for e in self.pg.edge_list:
            if len(e) < 3:
                print("S2VGraph, init, e:", e)
        ...
        for i in range(NUM_GRAMMAR_EDGE_TYPES):
            ...
            if len(edges) == 0:
                 continue # TESTING SKIPPING VALUES WHICH DO NOT HAVE EDGES
            print(str(i) + " " + str(edges))
